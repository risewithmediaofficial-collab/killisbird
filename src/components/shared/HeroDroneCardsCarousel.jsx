...
function HeroDroneCardsCarousel({ slides, className, autoplay = true, loop = true, dark = false }) {
          ...
          >
            <div className="relative flex h-full items-center justify-center">
              <motion.div
                ...
                className={cn(
                  "h-full w-full overflow-hidden rounded-[1.5rem] border p-4 shadow-[0_24px_50px_rgba(15,15,15,0.08)] sm:rounded-[1.75rem] sm:p-5",
                  dark
                    ? "border-white/8 bg-[linear-gradient(180deg,rgba(255,255,255,0.06),rgba(255,255,255,0.03))]"
                    : "border-border/60 bg-[linear-gradient(180deg,rgba(255,255,255,0.96),rgba(245,245,240,0.92))]"
                )}
              >
                <div className={cn(
                  "flex h-full flex-col rounded-[1.2rem] p-4 sm:rounded-[1.35rem] sm:p-5",
                  dark ? "bg-white/4" : "bg-white/78"
                )}>
                  <div className={cn("flex items-start justify-between gap-4", isMobile ? "hidden" : "")}>
                    <div>
                      ...
                      <h3 className={cn(
                        "mt-3 text-base font-semibold uppercase tracking-[0.08em] sm:text-xl",
      ...
      <div className="mt-5 flex flex-wrap items-center justify-between gap-4">
          ...
          {slides.map((slide, index) => (
            <button
              ...
              className={cn(
                ...
                current === index
                  ? dark ? "w-8 bg-white" : "w-8 bg-foreground"
                  : dark ? "w-2.5 bg-white/20" : "w-2.5 bg-foreground/15",
              )}
        ...
        <div className="ml-auto flex items-center gap-2">
          <button
            ...
            className={cn(
              "flex h-10 w-10 items-center justify-center rounded-full border transition-colors hover:border-neon/40 hover:text-neon",
              dark ? "border-white/12 bg-white/6 text-white" : "border-border/60 bg-white/88 text-foreground"
            )}
          >
            <ChevronLeft size={18} />
          </button>
