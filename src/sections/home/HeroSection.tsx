...
import { heroMetrics, tickerItems } from "@/data/home";
...
export function HeroSection() {
  ...
  // Scroll-based parallax for drone image — smooth, 12% drift
  ...
  return (
    ...
    >
      ...
      >
        <AnimatePresence mode="popLayout">
          <motion.img
            ...
            className="w-full h-auto object-contain animate-float-slow drop-shadow-2xl mix-blend-multiply dark:mix-blend-normal"
            draggable={false}
          />
        </AnimatePresence>
      </motion.div>
      ...
      {/* Main Hero Content */}
      <div className="container-edge relative z-20 w-full h-full flex flex-col justify-center pt-32 md:pt-40">
        {/* Left-Aligned Massive Typography */}
        <div className="relative w-full max-w-4xl flex flex-col items-start text-left z-10">
          <motion.div
          ...
          <motion.p
            ...
            className="max-w-[480px] mt-10 text-lg sm:text-xl text-muted-foreground font-medium leading-relaxed"
          >
            High-end aerial systems crafted for absolute precision, intelligent autonomy, and premium
            design.
          </motion.p>
          ...
          >
            <button
              ...
              className="px-10 h-16 rounded-full bg-neon text-black flex items-center justify-center shadow-neon hover:scale-105 transition-transform group gap-3 font-bold uppercase tracking-widest"
            >
              Explore
      ...
      >
            ...
            {Array.from({ length: 4 }).map((_, index) => (
              <div key={index} className="flex shrink-0 items-center">
                <span className="flex items-center gap-6 px-6">
                  DRONES <span className="text-neon/80">★</span>
                  PRECISION <span className="text-neon/80">★</span>
                  AERIAL <span className="text-neon/80">★</span>
                  AUTONOMOUS <span className="text-neon/80">★</span>
                  INNOVATION <span className="text-neon/80">★</span>
                  EXPLORE <span className="text-neon/80">★</span>
                </span>
              </div>
            ))}
          </div>
        ...
        <div className="absolute right-8 top-1/2 -translate-y-1/2 z-30 pointer-events-none hidden sm:block bg-black rounded-full p-1 shadow-2xl">
          <div
            className="w-14 h-14 rounded-full border border-neon flex items-center justify-center animate-spin"
            style={{ animationDuration: "10s" }}
          >
            <span className="text-neon text-[8px] font-bold text-center leading-tight">
              BUILT
              <br />
              FOR
              <br />
